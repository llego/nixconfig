...
#   cloud.cri.su  → OpenCloud  (0.0.0.0:9200, restricted to VPS Traefik)
...
in {
  ...
  # ── Firewall ─────────────────────────────────────────────────────────────
  # OpenCloud and Collabora listen on all interfaces so VPS Traefik can reach
  # them via Tailscale. These ports are not opened to the LAN.
  networking.firewall = {
    extraCommands = lib.mkAfter ''
      iptables -w -A nixos-fw -p tcp -s ${net.hosts.vps} --dport ${toString net.crisuflix.opencloud.port} -j nixos-fw-accept
      iptables -w -A nixos-fw -p tcp -s 100.0.0.0/8 --dport ${toString net.crisuflix.collabora.port} -j nixos-fw-accept
    '';
    extraStopCommands = lib.mkAfter ''
      iptables -w -D nixos-fw -p tcp -s ${net.hosts.vps} --dport ${toString net.crisuflix.opencloud.port} -j nixos-fw-accept 2>/dev/null || true
